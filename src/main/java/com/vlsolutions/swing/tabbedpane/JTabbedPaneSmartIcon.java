 ...
 *
 * <p>As JTabbedPanes cannot use any JComponents as tab selectors (the access if protected and we
 * just have a label, an icon and a tooltip), we have to rely on tricks to bypass them.
 *
 * @author Lilian Chamontin, VLSolutions
 * @update 2005/11/01 Lilian Chamontin : fixed NPE when otherIcons == null (tabs without smart
 *     icons)
 * @update 2005/11/08 Lilian Chamontin : fixed bug related to event management when multiple
 *     other-icons are used (big thanks to Emmanuel GAUVRIT).
...
public class JTabbedPaneSmartIcon implements Icon, Cloneable {
  ...
  /**
   * local tooltip text : the one to use during mouse movements (depends on the mouse position, and
   * can be either the tab selector tooltip text or one of the smart buttons included into this
   * icon).
   */
   ...
   *
   * <p>If the icon is shared between containers, please provide at least one as this allows the
   * icon to properly estimate its dimension.
   */
  ...
  public void paintIcon(Component c, Graphics g, int x, int y) {
    ...
    if (otherIcons != null) { // 2005/11/01
      for (int i = 0; i < otherIcons.length; i++) {
        otherIcons[i].paintIcon(
            c, g, x + iconsOffset, y + height / 2 - otherIcons[i].getIconHeight() / 2);
   ...
   * key-values. For each hint key present in that map, the value of that hint is obtained from the
   * Graphics and stored as the value for the key in savedHints.
   *
   * <p>From: http://docs.oracle.com/javase/7/docs/api/java/awt/doc-files/DesktopProperties.html
   */
    ...
    if (width == -1) {
      if (container == null) {
        throw new NullPointerException(
            "container for this smart icon not specified with setIconForTabbedPane()");
      }
      // reevaluate the width with correct graphics object, in case something has changed
  ...
  public int getIconHeight() {
    if (height == -1) {
      if (icon != null) {
        ...
        height =
            16; // standard height (as if there was an icon) : should be calculated instead of fixed
      }
    }
    return height;
  }
   ...
   *
   * <p>Mouse coordinates are given relative to this icon
   */
