 ...
 *
 * <p>This class is used (with a mouse listener) to forward events into the smart icon. which is
 * responsible for faking an enhanced tab selector (including label and optional buttons).
 *
...
public class JTabbedPaneSmartIconManager implements MouseListener, MouseMotionListener {
  ...
  /**
   * MouseListener implementation, use to track mouse behaviour inside the tab selector bounds and
   * forward them to the appropriate smart icon.
   */
   ...
   *
   * <p>There is always the risk that the last tab has been removed (and movedTab will be out of
   * bounds)
   */
      ...
      if (icon instanceof JTabbedPaneSmartIcon) {
        ...
        if (movedIcon != null && movedIcon != smartIcon) {
          ...
          if (movedIcon.onMouseExited(eSmart)) {
            ...
            //            tabbedPane.repaint(prevRect.x, prevRect.y, prevRect.width,
            // prevRect.height);
