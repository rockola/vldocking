...
public class ToolBarPanelLayout implements LayoutManager2 {
  ...
  private ToolBarConstraints getHorizontalInsertionContraintsAt(
      Component draggedComponent, Point p) {
    ...
    for (int i = 0; i < majorCount; i++) {
      ...
      if (p.y < bounds.y) { // insert before first
        ...
        return tc;
          ...
          > bounds.y + bounds.height) { // insert after.. ignore (will be tested on next loop)

      } else if (p.y
          > bounds.y
              + bounds.height
              - 5) { // insert a new major as we are just below the lower part
        tc.majorOrder = i;
        tc.minorOrder = 0;
        tc.majorOffset = +1;
        return tc;
    ...
    for (int i = 0; i < majorCount; i++) {
      ...
      if (p.x < bounds.x) { // insert before first
        ...
        return tc;
          ...
          > bounds.x + bounds.width) { // insert after.. ignore (will be tested on next loop)

      } else if (p.x
          > bounds.x + bounds.width - 3) { // insert a new major as we are just below the lower part
        tc.majorOrder = i;
        tc.minorOrder = 0;
        tc.majorOffset = +1;
        return tc;
