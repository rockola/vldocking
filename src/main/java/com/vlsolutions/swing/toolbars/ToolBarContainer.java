 ...
 *
 * <p>The layout used is BorderLayout and the CENTER component is dedicated to the user (the other 4
 * places (NORTH, EAST, WEST and SOUTH are used to display the toolbar panels)).
 *
 * <p align="center"><img src="doc-files/overview.gif"><br>
 * The ToolBarContainer is a JPanel with a BorderLayout, containing a user component and a maximum
 * of four ToolBarPanels.
 *
 * <p>Loading and saving toolbar configurations is done with the {@link ToolBarIO} utility class,
 * and works if VLToolbars are registered (given a name and made know to their container by {@link
...
public class ToolBarContainer extends JPanel {
   ...
   *
   * <p>Registering is used when reading a xml configuration
   */
  ...
  /**
   * Creates a default ToolBarContainer with preinstalled toolbar panels on the borders with the
   * LEADING alignment.
   *
   * <p>The toolbarPanels are then accessible with getToolBarPanelAt(constraints) where constraints
   * values are BorderLayout.NORTH, EAST, WEST and SOUTH.
   */
  public static ToolBarContainer createDefaultContainer(
    ...
    return createDefaultContainer(
        topToolbar, leftToolBar, bottomToolBar, rightToolBar, FlowLayout.LEADING);
  }

  /**
   * Creates a default ToolBarContainer with preinstalled toolbar panels on the borders with the
   * specified alignment The value of the alignment argument must be one of <code>FlowLayout.LEFT
   * </code>, <code>FlowLayout.RIGHT</code>, <code>FlowLayout.CENTER</code>, <code>
