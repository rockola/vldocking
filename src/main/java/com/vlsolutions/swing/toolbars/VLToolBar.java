 ...
 *
 * <p>To use the toolbar feature, you must follow this pattern :
 *
 * <ul>
 *   <li>The toolbars must be contained initially in a ToolBarPanel (which can contain multiple
 *       toolbars)
 *   <li>The ToolBarPanel must be contained in a ToolBarContainer (which can contain as much as 4
 *       ToolBarPanels , one on each border).
 *   <li>The center component of the ToolBarContainer can be the DockingDesktop, or any user
 *       component.
 * </ul>
 *
 * <p align="center"><img src="doc-files/overview.gif"><br>
 * The VLToolBar contains buttons and other components, and is included in a ToolBarPanel of a
 * ToolBarContainer .
 *
 * <p>Loading and saving toolbar configurations is done with the {@link ToolBarIO} utility class.
 *
 * <p>Toolbars can now be collapsed (to gain space on screen). this property is disabled by default
 * to keep compatibility with previous releases. For a better usage of collapsing it is better to
...
public class VLToolBar extends JPanel {
  ...
  /**
   * Constructs a toolbar with a null name (this toolbar will not be able to write its structure in
   * a XML encoder, unless you call setName(String) with a non null name before saving). ).
   */
  public VLToolBar() {
    this(null);
  }
   ...
   *
   * <p>Can be overriden to implement custom event handling.
   */
   ...
   *
   * <p>Default value is <b>false</b> to accomodate with VLButtonUI which paints itself the button
   * interiors.
   */
   ...
   *
   * <p>For a better user experience, please note to install an empty border of the same Insets than
   * this one (otherwise, there will be a size change of the toolbar at the beginning of the drag
   * and after the drop).
   */
  ...
  private boolean checkGripperOnOtherPanel(
      ToolBarPanel panel, MouseEvent e, Rectangle invisibleBounds) {
   ...
   *
   * <p>This method is called directly by the framework on toolbar installation or during drag and
   * drop.
   */
