 ...
 *
 * <p>When used in horizontal orientation, this layout respects the preferred width of the contained
 * components, and forces the height to the maximum of all the preferred heights. (so components are
 * vertically streched to have the same height).
 *
 * <p>When used in vertical orientation, this layout respects the preferred height of the contained
 * components, and forces the width to the maximum of all the preferred widths. (so components are
 * horizontally streched to have the same width).
 *
      ...
      if (isHorizontal) {
        ...
        for (int i = 0; i < components.size(); i++) {
          ...
          if (comp.isVisible()) {
            Dimension d = comp.getPreferredSize();
            comp.setBounds(
                insets.left, top, toolbarDim.width - insets.left - insets.right, d.height);
            top += d.height + gap;
          }
        }
      }
