 ...
 *
 * <p>The class uses the preferences framework of the java runtime, and installs a shutdown hook to
 * save the toolbars state on application exit.
 *
 * <p>Warning : this class cannot be run from a sandboxed (untrusted) environment (unsigned applet/
 * java web start) as it uses classes requiring a SecurityManager (Preferences API and Runtime
 * shutdown hook).
 *
 * <p>Call loadToolBarLayout() upon application startup, after registering all ToolBars.
 *
...
public class ToolBarPersistence implements Runnable {
  ...
  private static final String TOOLBAR_LAYOUT = "toolbar_layout"; // $NON-NLS-1$
   ...
   *
   * @param mainClass the main class of the application. Will be used to generate a reasonable name
   *     for the preferences to store toolbar configuration
  ...
  public ToolBarPersistence(Object mainClass, ToolBarContainer container) {
    ...
    String nodeName =
        mainClass
            ...
            .getName()
            .replaceFirst(
                mainClass.getClass().getPackage().getName() + ".", ""); // $NON-NLS-1$ //$NON-NLS-2$
