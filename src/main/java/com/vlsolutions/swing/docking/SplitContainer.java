 ...
 *
 * <p>The split container should be used only by the DockingDesktop.
 *
 * <p>It is a SplitPane with bug workarounds and a custom UI (in fact : no UI painting at all, by
 * default).
 *
...
public class SplitContainer extends JSplitPane implements DockDropReceiver {
  ...
  /**
   * Returns a suitable name for when this container is the main child of a Tab
  ...
  /**
   * Resest recursively the left and right components to their preferred size if possible. (same
   * contract as JSplitPane's).
   *
   * <p>If contained components are SplitContainers, their are also reset to their preferredSize.
   */
   ...
   * @param position where to put comp / child (i.e if SPLIT_TOP, a vertical splitpane will be
   *     created and comp will be on top of it (and child at bottom).
   */
  ...
  private void scanComponent(
      Component comp, DockEvent event, boolean drop, Point p, Rectangle compBounds) {
    ...
    if (min == distTop) {
      ...
      if (drop) {
        ...
        event
            .getDesktop()
            .splitComponent(
                comp, event.getDragSource().getDockable(), DockingConstants.SPLIT_BOTTOM);
      } else {
        Rectangle2D r2d =
      ...
      if (drop) {
        ...
        event
            .getDesktop()
            .splitComponent(
                comp, event.getDragSource().getDockable(), DockingConstants.SPLIT_RIGHT);
      } else {
        Rectangle2D r2d =
            new Rectangle2D.Float(
