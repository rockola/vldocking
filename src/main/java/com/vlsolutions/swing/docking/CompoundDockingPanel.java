 ...
 *
 * <p>This component is used by the API (as the associate component of a CompoundDockable) and
 * shouldn't be used outside this context.
 *
  ...
  private void scanContainer(DockEvent event, boolean drop) {
    ...
    event.setDockingAction(
        new DockingActionAddDockableEvent(
            event.getDesktop(), dragged, initialLocation, nextLocation, this));
    if (drop) {
      acceptDrop(event);
    } else {
