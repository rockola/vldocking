...
abstract class AbstractDragControler implements DragControler {
  ...
  protected boolean paintFloatingDragShape =
      UIManager.getBoolean("FloatingContainer.paintDragShape");
   ...
   *
   * <p>It can be used for example by a key listener reacting to the escape key
   *
   * @since 2.0.1
   */
  /**
  ...
  private boolean processMouseReleased(MouseEvent e) {
    ...
    DockingDesktop targetDesktop =
        umInfo.desktop; // findNearestNestedDesktop(underMouse);//umInfo.desktop
    ...
    if (underMouse == null) {
      // || isAncestorOf(dockableDragSource.getDockableContainer(), underMouse)){ 2007/01/08 moved
      // to implementations
      ...
      if (event.isDragAccepted()
          && process.isDockingActionAccepted()) { // triggers vetoable listeners
  ...
  protected void processMouseDragged(MouseEvent e) {
    ...
    if (underMouse
        == null) { // || isAncestorOf(dockableDragSource.getDockableContainer(), underMouse)){
      // it's not possible to drop a parent on one of its children
  ...
  protected void setDropShape(
      Shape shape, ShapePainterStrategy shapePainterStrategy, boolean floating) {
    if (dropShape == null) {
      if (shape != null) {
  ...
  class DragProcess {
    ...
    public DockDragEvent findAcceptableEvent(MouseEvent e) {
      ...
      do {
        ...
        MouseEvent convertMouse =
            SwingUtilities.convertMouseEvent((Component) e.getSource(), e, underMouse);
        DockableDragSource dragSource = ((DockableDragSource) e.getSource());
    ...
    private boolean checkSingleDockableWillBeDetached(Dockable dockable) {
      DockableState newState =
          new DockableState(desktop, dockable, DockableState.Location.FLOATING);
  ...
  class DropProcess extends DragProcess {
    ...
    private void fireSingleDockingActionEvent(Dockable dockable) {
      if (stateChange) {
        ...
        dockingContext.fireDockableStateChange(
            new DockableStateChangeEvent(
