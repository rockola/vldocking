 ...
 *
 * <p>The internal "key" will have to match the one of a DockKey when the worspace is applied to a
 * DockingContext.
 *
 * <p>example : if your application uses a dockable with DockKey k = new DockKey("testDockable");
 * <br>
 * than your corresponding WSDockKey will be wsdk = new WSDockKey("testDockable");
 *
 * <p>Design note : We use this class insted of DockKey because sometimes dockkeys are lazily
 * created (at dockable creation) and thus aren't already available when specifying a workspace.
 *
