 ...
 *
 * <p>Use this object to define the layout of a DockingDesktop. This layout can be applied later to
 * that desktop through the Workspace.apply(DockingContext ctx) method.
 *
 * <p>The API is a subset of the one of DockingDesktop : limited to layout building. you can
 * addDockable(), split(), createTab(), addHiddenDockable(), addFloatingDockable(), see these method
 * description for further information.
 *
 * <p>Node : WSDesktop doens't support layout updates : you should use it to build a layout, but not
 * to move dockables around. (if you add a dockable somewhere, don't add it elsewhere later as
 * removal isn't fully implemented).
 *
 * <p>In a WSDesktop, dockables are identified by a WSDockKey (a limited version of DockKey) : this
 * is to allow you to create workspaces with dockables that haven't been constructed yet (lazy
 * loading).
 *
 * <p>Example :
 *
...
public class WSDesktop {
  ...
  /**
   * The name of this desktop (which must match the name of a real desktop if more than one desktops
   * are used )
   */
   ...
   *
   * <p>Don't forget that this dockable MUST ALSO BE in the DOCKED state (added to the desktop in a
   * split, tab or compound node) otherwise the missing restore information will cause unwanted
   ...
   *     split(a, b, SPLIT_TOP, 0.1f) and split(a, b, SPLIT_BOTTOM, 0.1f) have the same dividor
   *     location (somewhere near the top of the split).
   */
  public void split(
      WSDockKey base, WSDockKey newDockable, DockingConstants.Split split, double splitLocation) {
  ...
  /**
   * Creates a tab containing baseTab and newTab (if baseTab is already into a Tab, then newTab will
   * just be added at the "order" position.
   */
  ...
  public void setFloating(
      WSDockKey dockable, Rectangle windowRect, RelativeDockablePosition returnPosition) {
    FloatingNode f = new FloatingNode(new SingleDockableNode(dockable), windowRect, returnPosition);
    floatingNodes.add(f);
  }

  /**
   * Returns the name of this desktop (which must match the name of a real desktop if more than one
   * desktops are used )
   */
  ...
  private void xmlWriteSplit(SplitNode splitNode, PrintWriter out) {
    ...
    int orientation =
        splitNode.isHorizontal ? JSplitPane.HORIZONTAL_SPLIT : JSplitPane.VERTICAL_SPLIT;
  ...
  private void xmlWriteFloatingDockable(
      SingleDockableNode dockable, PrintWriter out, RelativeDockablePosition returnPosition) {
    ...
    out.println(
        "<RelativePosition x=\""
            ...
            + returnPosition.getY()
            + "\" w=\""
            + returnPosition.getWidth()
            + "\" h=\""
            + returnPosition.getHeight()
            + "\" />");
    out.println(
        "<PreviousState state=\""
            + DockableState.Location.DOCKED.ordinal()
  ...
  private void xmlBuildRootNode(org.w3c.dom.Node node)
  ...
  private Node xmlCreateComponent(Element elt, DockableState.Location dockableLocation)
      throws SAXException {
  ...
  private void xmlBuildCompoundDockable(
      CompoundDockableNode cdn, Element elt, DockableState.Location dockableLocation)
      throws SAXException {
  ...
  private SplitNode xmlBuildSplitContainer(Element elt, DockableState.Location dockableLocation)
      throws SAXException {
  ...
  private TabNode xmlBuildTabbedDockableContainer(
  ...
  private void xmlBuildAutoHideNode(int zone, Element dockableElt) throws SAXException {
    ...
    if (dockable instanceof CompoundDockableNode) {
      ...
      xmlBuildCompoundDockable(
          (CompoundDockableNode) dockable, dockableElt, DockableState.Location.HIDDEN);
    }
  }
  ...
  private void xmlBuildFloatingNode(NodeList dockables, Rectangle rectangle) throws SAXException {
    ...
    for (int i = 0; i < dockables.getLength(); i++) {
      ...
      if (dockable instanceof CompoundDockableNode) {
        // check for children and build them
        xmlBuildCompoundDockable(
            (CompoundDockableNode) dockable, dockableElt, DockableState.Location.FLOATING);
      }
    }
  }
