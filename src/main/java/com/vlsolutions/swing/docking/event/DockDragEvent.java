 ...
 *
 * <p>Standard processing of dragging is (ignoring DockableStateChange events) :
 *
 * <ul>
...
public class DockDragEvent extends DockEvent {
   ...
   *
   * <p>This is useful for nested DropReceivers (like TabbedDockableContainers)
   */
   ...
   *
   * <p>This method will be invoked only if <code>isDragAccepted</code> returns true
   *
   * <p>As this method can be called on every mouse drag over a component, it is a good practice to
   * reuse the same shape as lond a possible (do not create a new Shape at every invocation).
   */
  ...
  /**
   * Used by a DockDropReceiver to indicate that the drag operation is accepted (mouse is over a
   * droppable zone).
   *
   * <p>As of version 2.1, please note that you also have to invoke #setDockingAction() to specify
   * the action associated with the drag shape.
   *
   * @param dropShape the shape (in drop component coordinates) showing the drop zone, which will be
   *     displayed on the glasspane of the DockingPanel.
   */
   ...
   *
   * <p>for example, a TabbedDockableContainer that doesn't allow drops on its borders can delegates
   * this management to its container.
   */
