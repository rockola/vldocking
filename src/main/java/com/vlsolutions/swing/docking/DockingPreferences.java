 ...
 *
 * <p>It acts as a facade for installing common settings.
 *
 * <p>This class contains only high-level settings, like the heavyweight components support or the
 * global desktop style (shadowed or flat). To access low-level properties, use the
 * DockingUISettings class instead.
 *
...
public class DockingPreferences {
  ...
  /**
   * A simple utility method to configure the docking framework to work properly with heavyweight
   * component.
   *
   * <p>Don't forget to invoke also the following general swing methods :
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>If you are sure you will always use heavyweight component, you can replace this method
   * invocation by initHeavyWeightUsage(), which will do all the work for you (even the general
   * swing methods invocation).
   */
   ...
   *
   * <p>This allows optimizations and workarounds (for properly using the JDIC WebBrowser for
   * example).
   */
  ...
  /**
   * Convenience method to use the VLDocking 1.0 layout and border style (with shadows around
   * dockables)
   */
   ...
   *
   * <p>The new "flat style" uses light borders around dockables and top tab placements.
   *
   * <p>This style is the new default one, so there is no need to call this method unless you want
  ...
  public static void setCustomSplitDesktopStyle(
      BufferedImage horizontalTile, BufferedImage verticalTile) {
