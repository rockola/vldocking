 ...
 *
 * <p>This class is not inteded for API users, it should be let to framework developpers.
 *
...
public class DockingUtilities {
  ...
  /**
   * Returns the first TabbedDockableContainer which is parent of this dockable component, or null
   * is there isn't any.
   */
  ...
  private static void swapComponents(
      SplitContainer split1, SplitContainer split2, Component comp1, Component comp2) {
  ...
  public static void replaceChild(Container parent, Component child, Component newChild) {
    // System.out.println("replace child : PARENT=" + parent + ", CHILD=" + child + ", NEW=" +
    // newChild);
  ...
  /**
   * Returns the split pane containing this dockable (if any), or null if this dockable isn't
   * contained in a splitpane.
   *
   * <p>If the dockable is nested in a TabbedDockableContainer, the split pane returned will be the
   * one containing the tabbed container (if any).
   */
  public static SplitContainer getSplitPane(Dockable dockable, int orientation) {
    ...
    while (comp != null && !found) {
      if (comp instanceof SplitContainer
          && ((SplitContainer) comp).getOrientation() == orientation) {
        found = true;
      } else {
        comp = comp.getParent();
      }
   ...
   *
   * <p>This method may return null if no SingleDockableContainer ancestor is found.
   */
  ...
  public static void setLocationRelativeTo(
      FloatingDockableContainer fdc, Component rel) { // 2006/02/20
  ...
  /**
   * returns the root pane used by this detached dockable container, regardless of its type (frame
   * or dialog)
   */
   ...
   *
   * <p>If the compound dockable contains another compound dockable this one is also added, along
   * with its own children.
   *
   * @return an ArrayList of Dockable
   */
  public static ArrayList<Dockable> findCompoundDockableChildren(
      CompoundDockable compoundDockable) {
  ...
  /**
   * searches up the dockable container hierarchy and returns the first ancestor which is a
   * CompoundDockable (or null if not found).
   */
  ...
  /**
   * searches up the dockable container hierarchy and returns the dockable state of the <b>last<b>
   * (top most) ancestor which is a CompoundDockable (or null if not found).
   */
   ...
   *
   * <p>Two states are currently managed : docked and floating (not hidden/maximized/closed).
   */
