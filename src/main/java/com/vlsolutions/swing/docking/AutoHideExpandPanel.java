 ...
 *
 * <p>Auto-Hide / Auto-Expand feature is a means to save space of screen replacing a Dockable by a
 * Button on one of the Desktop borders.
 *
 * <p>When the user clicks on the button or his mouse rolls over it, the component is shown (with an
 * expansion animation) as if it was coming from behind the button's border.
 *
 * @author Lilian Chamontin, vlsolutions.
 * @version 2.0
 * @update 2005/10/06 Lilian Chamontin : protected access to the exansion timer that might be null
 *     when AutoHidePolicy is EXPAND_ON_CLICK
 * @update 2005/11/01 Lilian Chamontin : enhanced timer management to auto-collaspe the panel when
 *     mouse out of bounds
...
public class AutoHideExpandPanel extends JPanel implements SingleDockableContainer {
  ...
  private JPanel topDragger =
      getTopDragger(); // indirection because those components are used by the ui delegate
  ...
  /**
   * this boolean is used to disable auto-hiding temporarily, especially during drag operation
   * (where mouse can leave the component)
   */
        ...
        try {
          ...
          Method m =
              Container.class.getMethod(
                  "setComponentZOrder", new Class[] {Component.class, int.class});
   ...
   *
   * <p>During drag operations (resizing), some mouseEnter/mouseExit events can be lost. In that
   * case, the desktop relies on this method to request collaping or not.
   */
   ...
   *
   * <p>This is useful when re-installing a desktop from readXml (for example, it resets dockable
   * expand sizes)
   */
  public void clear() {
    // clear
