 ...
 *   <li>layout policy is WRAP_TAB_LAYOUT (due to a java bug which limits usage of
 *       MouseMotionListeners in JTabbedPanes, it is not possible to use SCROLL_LAYOUT_POLICY)
 * </ul>
 *
 * <p>This tabbed pane is not meant to be used outside of DockingDesktop because of their connected
 * behaviour (drag/drop support, docking constraints, etc).
 *
 * <p>As of version 2.0 of the framework, the tabbed pane displays a close icon (other functions are
 * available with a right click menu).
 *
 * @see DockingDesktop
 * @author Lilian Chamontin, vlsolutions.
 * @version 1.0
 * @update 2005/11/01 Lilian Chamontin : enhanced shape painting (tries to avoid going outside the
 *     tab bounds)
 * @update 2005/11/10 Lilian Chamontin : added support for DnD multiple tabs at once.
 * @update 2005/12/08 Lilian Chamontin : removed a portion of code as its been refactored in
 *     BorderSplitter
...
public class DockTabbedPane extends JTabbedPane
  ...
  private PropertyChangeListener keyChangeListener =
        ...
        public void propertyChange(PropertyChangeEvent e) {
          ...
          if (e.getPropertyName().equals(DockKey.PROPERTY_CLOSEABLE)) {
            ...
            if (tab >= 0) {
              ...
              setToolTipTextAt(
                  tab, (String) e.getNewValue()); // 2008/04/15 also propagate to the tab
            }
          }
        }
      };
  ...
  private void scanMainTabZone(
      DockEvent event, boolean drop, DockableDragSource dragSource, Rectangle vbounds) {
    ...
    if (drop) {
      ...
      // event.getDesktop().addToTabbedGroup(base, event.getDragSource().getDockable());
      // //2005/07/13
  ...
  private boolean scanBorderBounds(DockEvent event, boolean drop, Point p) {
    ...
    if (firstDockable.getDockKey().getLocation() == DockableState.Location.FLOATING) {
      // as of v2.1 this DnD operation is allowed if the tabbed pane is a child of a compund
      // dockable
    ...
    if (getTabPlacement() == SwingConstants.BOTTOM) {
      ...
      innerBounds =
          new Rectangle(20, yTab + 20, getWidth() - 20 - 20, getHeight() - yTab - 20 - 20);
    }
