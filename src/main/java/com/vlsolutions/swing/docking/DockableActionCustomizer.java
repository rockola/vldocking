 ...
 *
 * <p>It can be used, for example, to add a set of menu items to the contextual menu associated with
 * any dockable.
 *
 * <p>Example :
 *
 ...
 * </pre>
 *
 * <p>Implementation Note : the customizer is invoked every time a pop-up is triggered from the
 * associated dockable container. It is a good practice to keep object creation and listener
 * attachments outside the visit..() methods.
 *
...
public class DockableActionCustomizer {
  ...
  /**
   * Override this method to add contextual items at the end of the pop-up menu which can be
   * triggered on a tab selector.
   *
   * <p>It is recommended to add menu items at the end to the pop-up menu as other items may have
   * been installed by other visitors (DockGroup for example)
   *
   * <p>implementation note : This method can be called multiple times as pop-up menus are built
   * during the contextual event dispatch (not stored for the life of the dockable).
   */
  public void visitTabSelectorPopUp(JPopupMenu popUpMenu, Dockable tabbedDockable) {}

  /**
   * Override this method to add contextual items at the end of the pop-up menu which can be
   * triggered on the title bar of a single dockable.
   *
   * <p>It is recommended to add menu items at the end to the pop-up menu as other items may have
   * been installed by other visitors (DockGroup for example)
   *
