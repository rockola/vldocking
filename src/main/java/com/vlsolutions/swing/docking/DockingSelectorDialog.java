 ...
 *
 * <p>This dialog shows which user components are docked, auto-hidden, not visible, and is capable
 * of dynamicaly altering the dockingpanel layout on user selection (for example, to show an
 * unvisible component).
 *
 * <p>User components shown are those which are already docked and those which have been registered
 * with the registerDockableComponent() method of DockingPanel.
 *
 * <p>Example of usage :
 *
 * <pre>
...
public class DockingSelectorDialog extends JDialog {
  ...
  private HashMap<DockableState, Boolean> visibleViews =
      new HashMap(); // DockableState / Boolean (visible?)

  /**
   * Default constructor.
   *
   * <p>modal dialog with DISPOSE_ON_CLOSE option, needs a pack() and setVisible(true) after
   * creation
   */
  ...
  private void init() {
    ...
    jsp.setBorder(
        BorderFactory.createCompoundBorder(
            BorderFactory.createEmptyBorder(0, 5, 0, 5), jsp.getBorder()));
   ...
   *
   * <p>Use this method if you need to know how is was closed. If the used has confirmed, the
   * docking panel is automaticaly updated by the confirm button action.
   */
