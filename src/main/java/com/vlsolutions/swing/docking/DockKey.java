...
/**
 * Provides a unique identification for a Dockable and runtime properties, like icon, name, tooltip,
 * preferred audohide-zone...
 *
 * <p>As it is the object which <b>identifies uniquely a user Dockable component</b>, it should be
 * associated with one and only one <code>Dockable</code>.
 *
 * <p>The unique key used for equals() comparison is the <b>dockKey</b> property, other properties
 * can be shared by a set of DockKey (for example, you can share an icon, or even a display name
 * between dockable Components).
 *
 * <p>Another usage is the decoration of a dockable container, providing informations such as its
 * name, tooltip or icon.
 *
 * <p>Properties of a DockKey are listened to by the docking framework, so a change of name or icon
 * is reflected on the GUI without further programming.
 *
...
public class DockKey {
  ...
  /**
   * resize weight of the dockable, this is not a bound property and should be set during
   * initialization.
   */
  ...
  /**
   * Current visibility dockableState of the dockable (DockableState.CLOSED, STATE_AUTO_HIDE,
   * DOCKED, MAXIMIZED, FLOATING)
   */
  ...
  /**
   * Constructs a DockKey with dockKey (unique key), a displayed name, a tooltip, an icon and a
   * default autohide border.
   */
  public DockKey(
      String dockKey, String name, String tooltip, Icon icon, DockingConstants.Hide hideBorder) {
   ...
   *
   * <p>Note : This used to be the getDockName prior version 2.0. It has been renamed to clarify the
   * concept (there was a naming problem between getName() and getDockName()
   */
  ...
  /**
   * Updates the name property. The name property is used by dockable container headers to associate
   * a title with a dockable.
   *
   * <p>PropertyListeners are notified of that change
   */
  ...
  /**
   * Returns the tab name (or tab title) displayed when the component is contained into a tabbed
   * container.
   */
  ...
  /**
   * Updates the tabname property. This property is used by tabbed containers to display a shorter
   * version of the title of this dockable.
   *
   * <p>Default value is null, meaning the name
   *
   * <p>PropertyListeners are notified of that change
   *
   * @since 2.1
   ...
   *
   * <p>Although PropertyListeners are notified of that change, it is not recommended to change
   * dynamicaly the dockKey property, as it is heavily used in the docking framework to identify
   * dockable components.
   *
   * <p>Note : This used to be the getDockName prior version 2.0. It has been renamed to clarify the
   * concept (there was a naming problem between getName() and getDockName()
  ...
  /**
   * Updates the notification propety. Notification results in a visual change of the dockable in
   * order to attract attention from the user to this dockable.
   *
   * <p>PropertyListeners are notified of that change.
   */
   ...
   *
   * <p>Warning : do not call this method, it is for the sole use of the DockingDesktop API.
   *
   * @see DockableState
   */
