...
class HeavyWeightDragControler extends AbstractDragControler {
  ...
  private boolean paintBackgroundUnderDragRect =
      UIManager.getBoolean("DragControler.paintBackgroundUnderDragRect");
   ...
   *
   * <p>As painting is different when pure Swing is used (glasspane) and heavyweight components are
   * mixed in (glasspane + canvas).
   */
  private class HWShapePainterStrategy implements ShapePainterStrategy {

    private DragControlerGlassPane dragGlassPane =
        new DragControlerGlassPane(HeavyWeightDragControler.this);
  ...
  private class HeavyShape extends Canvas {
    ...
    public void paint(Graphics g) {
      if (dropShape != null) {
        ...
        Shape s =
            AffineTransform.getTranslateInstance(-shapeX, -shapeY)
