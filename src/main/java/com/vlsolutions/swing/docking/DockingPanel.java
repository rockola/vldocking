 ...
 *
 * <p>Users of the VLDocking Framework should not call this class which is a utility component of
 * DockingDesktop.
 *
  ...
  private void acceptDrag(DockEvent event, DockingConstants.Split position, Shape shape) {
    ...
    if (dragContainer instanceof TabbedDockableContainer) {
      event.setDockingAction(
          new DockingActionSplitDockableContainerEvent(
  ...
  private void scanContainer(DockEvent event, boolean drop) {
    ...
    if (min == distTop) {
      ...
      if (drop) {
        ...
        Rectangle2D r2d =
            new Rectangle2D.Float(compBounds.width - bestWidth, 0, bestWidth, compBounds.height);
