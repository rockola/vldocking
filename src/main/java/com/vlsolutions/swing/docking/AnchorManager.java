 ...
 *
 * <p>Constraints are currently defined as anchors (AnchorConstraints objects associated to
 * Dockables).
 *
...
public class AnchorManager {
  ...
  private HashMap<Dockable, AnchorConstraints> constraintsByDockable =
      new HashMap<Dockable, AnchorConstraints>();
  ...
  private DockingActionListener actionListener =
        ...
        public boolean acceptDockingAction(DockingActionEvent event) {
          switch (event.getActionType()) {
            ...
            case DockingActionEvent.ACTION_SPLIT_DOCKABLE_CONTAINER:
              {
                DockingActionSplitDockableContainerEvent e =
                    (DockingActionSplitDockableContainerEvent) event;
   ...
   *
   * @param context the context (can be taken from DockingDesktop.getDockingContext() used by this
   *     manager
   * @param container the "top level" container managed (usually a DockingDesktop or a
   *     CompoundDockingPanel)
   */
  ...
  private boolean acceptSplitComponent(DockingActionSplitComponentEvent event) {
    ...
    // we suppose that dockables in "base" are ok (respecting their own set of constraints before
    // the split)
