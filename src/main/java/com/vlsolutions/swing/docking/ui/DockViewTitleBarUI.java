...
public class DockViewTitleBarUI extends PanelUI implements PropertyChangeListener {
  ...
  private static Icon maximizeIconRollover =
      UIManager.getIcon("DockViewTitleBar.maximize.rollover");
  ...
  // flags to hide/show buttons in the title bar (they are always visible in the contextual menu,
  // but might
  ...
  private boolean isCloseButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isCloseButtonDisplayed");
  private boolean isHideButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isHideButtonDisplayed");
  private boolean isDockButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isDockButtonDisplayed");
  private boolean isMaximizeButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isMaximizeButtonDisplayed");
  private boolean isRestoreButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isRestoreButtonDisplayed");
  private boolean isFloatButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isFloatButtonDisplayed");
  private boolean isAttachButtonDisplayed =
      UIManager.getBoolean("DockViewTitleBar.isAttachButtonDisplayed");
  ...
  public void installUI(JComponent c) {
    ...
    c.setCursor(
        Cursor.getDefaultCursor()); // needs this to avoid artifacts with floating dockable borders.
  }
  ...
  public void propertyChange(PropertyChangeEvent e) {
    ...
    if (pName.equals("dockable")) {
      ...
      boolean isMaximized =
          titleBar.getDockable().getDockKey().getLocation() == DockableState.Location.MAXIMIZED;
      if (isAutoHideable) {
        if (!isMaximized) {
      ...
      boolean isHidden =
          titleBar.getDockable().getDockKey().getLocation() == DockableState.Location.HIDDEN;
      if (isMaximizeable) {
        if (!isHidden) {
  ...
  /**
   * Update the buttons to track state changes (for example, the maximize button can become
   * "restore" when the view is maximized.
   */
  ...
  /**
   * Custom title bar painting : uses a gradient from the background color to the control highlight
   * color.
   */
