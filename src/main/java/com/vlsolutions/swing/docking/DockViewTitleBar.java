 ...
 *
 * <p>Here is an example of a title bar : <img src="doc-files/titlebar.gif">.
 *
 * <p>DockViewTitleBar is able to display the following properties of a DockKey :
 *
 * <ul>
 *   <li>Icon
 *   <li>Name
 ...
 * </ul>
 *
 * <p>This title bar supports buttons used for docking features :
 *
 ...
 * </table>
 *
 * <p>The buttons managed have no effect on the state of the dockable : they just fire property
 * change events, and it is the responsibility of the DockableContainer to listen to those events
 * and to relay the operation to the docking desktop.
 *
 * <p>Note : the UI Delegate of the DockViewTitleBar is the {@link
...
public class DockViewTitleBar extends JPanel implements DockableDragSource {
  ...
  private static final String ICONIFY_TEXT =
      UIManager.getString("DockViewTitleBar.minimizeButtonText");
  private static final String RESTORE_TEXT =
      UIManager.getString("DockViewTitleBar.restoreButtonText");
  private static final String MAXIMIZE_TEXT =
      UIManager.getString("DockViewTitleBar.maximizeButtonText");
  ...
  private static final String ATTACH_TEXT =
      UIManager.getString("DockViewTitleBar.attachButtonText");
   ...
   *
   * <p>Warning : a DockViewTitleBar can be used with multiple dockables (this is the case for
   * example in autohide borders, where a single titlebar is shared by all hidden dockables (shown
   * only when one is expanding).
   */
   ...
   *
   * <p>This hook can be used to insert customized buttons without otherwise having to fully replace
   * the UI delegate
   */
   ...
   *
   * <p>Shouldn't be used to update the title : the best way is to update the DockKey (property
   * listener ensure the labels and buttons stay in sync).
   *
   * @since 2.0
   */
   ...
   *
   * <p>As hiding and docking are mutually exclusive, the same button is used for both purposes
   */
   ...
   *
   * <p>As those operations are mutually exclusive, the same button is used for both purposes
   */
  ...
  /**
   * This method is invoked to hide the pop-up that could still be visible (To avoid a visible
   * pop-up for an invisible component)
   */
   ...
   *
   * <p>There is at most one active dockable for a dekstop, and it there is one, it is the one which
   * contains the keybord focused component.
   */
  ...
  /**
   * Updates the active property. A title bar is active when the dockable it is for is ancestor of
