 ...
 *
 * <p>When a dockable is associated to a DockGroup (via its DockKey) drag and drop operations are
 * limited to dockables of the same group (or new location) or compatible group (a group with a
 * common ancestor see {@link #isCompatibleGroup(DockGroup)} ).
 *
 * <p>It becomes easy to develop, for example, and MDI program : all documents are given the same
 * DockGroup, and other dockable are given another group so they don't mess with each other.
 *
 * <p>The DockGroups can be nested to allow some mixing (for example, an editor application can
 * consist of 4 groups : "Formaters" and "Files" the parent of "TextFiles" and "HTMLfile". In that
 * case, Textfiles and Htmlfiles can be part of tha same tabbed container whereas Formaters are kept
 * outside.
 *
...
public class DockGroup {
   ...
   *
   * <p>Note that names must be unique (this feature is not controlled by the framework) through all
   * DockGroups.
   */
  ...
  /**
   * returns true if this group is the ancestor of group g. To be an ancestor, this group must be
   * either equel to g, or to one of g's parents chain.
   */
   ...
   *
   * <p>two groups are compatible if they share the same ancestor or if one is the ancestor of the
   * other.
   *
   * <p>When compatibility is found, dockables of those groups can be docked in the same tab
   * container.
   */
