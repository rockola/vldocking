...
/**
 * An abstract factory used by the <code>DockingDesktop</code> to create instance of Dockable
 * containers.
 *
 * <p>This factory enables API Extenders to change part of the behaviour on the DockingDesktop
...
public abstract class DockableContainerFactory {
  ...
  /**
   * This method is called by the DockingDesktop whenever a dockable is inserted in its containment
   * hierachy.
   */
  public abstract SingleDockableContainer createDockableContainer(
      Dockable dockable, ParentType parentType);

  /**
   * @deprecated use the other createDockableContainer method ({@link
  ...
  /**
   * This method is called when a dockable is detached from the DockingDesktop and put in the
   * FLOATING state.
   *
   * <p>The floating container must be an instanceof Dialog or Window.
   */
   ...
   *
   * <p>This method is used by the standard implementation of Docking (DockView, etc) but is
   * optional for other implementations, as currently the titlebar isn't specified as a core
   * component of the docking framework (i.e. it is not an interface).
   *
   * @since 2.1.3
   */
   ...
   *
   * <p>it no factory has been provided, it will fall back on DefaultDockableContainerFactory
   */
   ...
   *
   * <p>Note that this change is not propagated to already docked components.
   */
