...
/**
 * This class is an immutable object used to remember the relative positionning of docked
 * components.
 *
 * <p><code>x, y, w, h</code> fields are expressed with <code>double</code> values varying from 0 to
 * 1.
 *
 * <p>
 *
 ...
 * </ul>
 *
 * <p>Objects of this class are used to reposition a Dockable on the desktop, based on lazy
 * constraints : As the desktop is a mix of horizontal and vertical split panes ({@link
 * SplitContainer}) it is not always possible to put a dockable exactly where you want given a set
 * of x,y, w, and h constraints.
 *
...
public class RelativeDockablePosition {
  ...
  public static final RelativeDockablePosition TOP_CENTER =
      new RelativeDockablePosition(0.4, 0, 0.2, 0.2);
  ...
  public static final RelativeDockablePosition TOP_RIGHT =
      new RelativeDockablePosition(0, 0, 0.2, 0.2);
  ...
  public static final RelativeDockablePosition TOP_LEFT =
      new RelativeDockablePosition(0.8, 0, 0.2, 0.2);
  ...
  public static final RelativeDockablePosition BOTTOM_CENTER =
      new RelativeDockablePosition(0.4, 0.8, 0.2, 0.2);

  /** convenience field referencing the TOP-RIGHT corner of the DockingDesktop */
  public static final RelativeDockablePosition BOTTOM_LEFT =
      new RelativeDockablePosition(0, 0.8, 0.2, 0.2);
  ...
  public static final RelativeDockablePosition LEFT_CENTER =
      new RelativeDockablePosition(0, 0.4, 0.2, 0.2);

  private double x, y, w; // relative positionning (0-1) of the component in the container

  /**
   * The container used for relative positioning (can be a top level DockingPanel or an intermediate
  ...
  /**
   * @see AnchorConstraints.ANCHOR_TOP (this field is an ORing of possible anchors) and contains 4
   *     boolean values (TOP, LEFT, BOTTOM, RIGHT)
   */
  private int anchors;

  /**
   * Construct a RelativeDockablePosition, assigning x, y, w, h values from the current position of
   * a dockable in its desktop container.
   *
   * <p>This constructor is a convenience for the framework, and should not be used by API users
   * (thus is package protected visibility).
   */
   ...
   *
   * <p>Please remember that the values must be between 0 and 1, as they designate relative
  ...
  public RelativeDockablePosition(
      Container relativeAncestorContainer, double x, double y, double w, double h) {
   ...
   *
   * <p>This can be a DockingPanel (top level ancestor) or CompoundDockingPanel (intermediate
   * ancestor)
   */
  ...
  /**
   * Returns an integer value representing the anchoring of the dockable relatively to its ancestor
   * container.
   *
   * <p>For example a dockable at the top left angle of an ancestor container will have an "anchors"
   * value or : AnchorConstraints.ANCHOR_TOP|AnchorConstraints.ANCHOR_LEFT
   *
    ...
    if (dockableContainer != null && relativeAncestorContainer != null) {
      ...
      Rectangle converted =
          SwingUtilities.convertRectangle(
              c, new Rectangle(0, 0, c.getWidth(), c.getHeight()), relativeAncestorContainer);
      ...
      this.anchors =
          RelativeDockingUtilities.findAnchors(dockable.getComponent(), relativeAncestorContainer);
    }
  }
}
