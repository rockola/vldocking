 ...
 *
 * <p>It is the equivalent of what is JDesktopPane for JInternalWindow : a JLayeredPane customized
 * to include :
 *
 * <UL>
 ...
 *   <LI>An abstraction of the Dockable notion, with factories to allow different implementations or
 *       customization of the Dockable containers.
 ...
 * @update 2005/11/08 Lilian Chamontin : added support for global width/height drop (in split
 *     method)
 * @update 2005/11/14 Lilian Chamontin : reworked setFloating methods.
 * @update 2005/12/08 Lilian Chamontin : fixed a bug related to multiple desktop usage (when moving
 *     a window after the desktop has been removed from hierarchy, the listeners were still invoked
 *     and caused a NPE in moveFloatingWindows).
 * @update 2006/12/01 Lilian Chamontin : fixed a NPE when closing a window with some floating
 *     children remaining
 * @update 2006/12/19 Lilian Chamontin : fixed a memory leak issue due to keyboard focus manager
 *     handling
 ...
 * @update 2008/07/05 fixed a NPE occuring when saving workspace and a tabbed component was still
 *     maximized
 */
...
public class DockingDesktop extends JLayeredPane {
  ...
  protected AutoHideButtonPanel topBorderPane =
      new AutoHideButtonPanel(expandPanel, DockingConstants.INT_HIDE_TOP);
  /** panel used to display autohide buttons */
  protected AutoHideButtonPanel leftBorderPane =
      new AutoHideButtonPanel(expandPanel, DockingConstants.INT_HIDE_LEFT);
  /** panel used to display autohide buttons */
  protected AutoHideButtonPanel bottomBorderPane =
      new AutoHideButtonPanel(expandPanel, DockingConstants.INT_HIDE_BOTTOM);
  /** panel used to display autohide buttons */
  protected AutoHideButtonPanel rightBorderPane =
      new AutoHideButtonPanel(expandPanel, DockingConstants.INT_HIDE_RIGHT);

  /** array containing the border panes */
  protected AutoHideButtonPanel[] borderPanes = {
    topBorderPane, leftBorderPane, bottomBorderPane, rightBorderPane
  };
  ...
  private MaximizedComponentReplacer dummyMaximedReplacer =
      new MaximizedComponentReplacer(); // 2007/01/18
  // new JLabel();
  ...
  /**
   * this timer is only used when the java version is < 1.5 (version>=1.5 uses the MouseInfo
   * component)
   */
  ...
  protected HashMap<Dockable, LinkedList<Dockable>> tabbedGroups =
      new HashMap(); // <Dockable>/<LinkedList<Dockable>
   ...
   *
   * <p>This also constructs a new Docking Context, that can be shared with other Desktops.
   */
  ...
  public DockingDesktop(String desktopName, DockingContext context) {
    ...
    mouseMotionGrabber.addMouseListener(
        new MouseAdapter() {
            ...
            if (event.getComponent() == DockingDesktop.this) {
              ...
              if (maximizedComponent != null) {
                if (DockingPreferences.isLightWeightUsageEnabled()) {
                  maximizedComponent.setBounds(
                      i.left, i.top, w - i.left - i.right, h - i.top - i.bottom);
                } else {
  ...
  /**
   * Registers listeners for notable keyboard events (CLOSE, DOCK/HIDE, MAXIMIZE/RESTORE,
   * float/attach). KeyStroke bindings can be defined and overriden this UIManager properties (see
   * DockingUISettings for details).
   */
   ...
   *
   * <p>A selected dockable is a dockable whose component contains the keyboard focus.
   */
   ...
   *
   * <p>Transparent contents allow adding background images/information when no dockable is visible
   * (everything closed or autohidden).
   *
   * <p>Default value is true (opaque).
   *
   * @since 2.0.4
   */
   ...
   *
   * <p>Usage of this method should be limited to VLDocking extensions (simple users shouldn't rely
   * on the underlying DockingPanel existence)
   */
   ...
   *
   * <p>Registration is automatic for shown dockables : methods setCentralDockable(), split(),
   * createTab(), hide()... leading to have the dockable visible call registerDockable() )
   *
   * <p>for not shown dockable (in order to read a configuration from an XML stream, or to list the
   * dockable in DockingSelectorDialog ), this method must be called manually.
   *
   * <p>As of version 2.1, this method call is forwarded to the DockingContext
   */
  ...
  /**
   * Unregisters the dockable, which can be garbage collected (no longer used by the docking
   * desktop.
   *
   * <p>As of version 2.1, this method call is forwarded to the DockingContext
   */
  ...
  /**
   * Returns a String containing the version of the docking framework in the format M.m.r where M is
   * the major , m the minor and r the release.
   *
   * @since 2.0
   */
   ...
   * @param dockable a dockable to add at the same position than <code>base</code>. if base is not
   *     already child of a tabbedpane, a new tabbedpane will be created and inserted at base's
   *     location.
   ...
   *
   * @param base an existing dockable, either displayed in a DockableContainer or in a
   *     TabbedDockableContainer.
   *     <p>If base is displayed by a DockableContainer, this container will be replaced by a
   *     TabbedDockableContainer.
  ...
  /**
   * private implementation wich allows for triggering state change event or not (depending on the
   * caller).
   */
  private void createTab(
      Dockable base, Dockable dockable, int order, boolean select, boolean triggerEvents) {
    /*
    ...
    DockingActionEvent dae =
        new DockingActionCreateTabEvent(
            this, dockable, currentLocation, newState.getLocation(), base, order);
    ...
    TabbedDockableContainer baseTab =
        DockingUtilities.findTabbedDockableContainer(base); // 2005/05/19
   ...
   *
   * <p>The base dockable is the reference, the second newDockable will be added according to the
   * position parameter.
   *
   * <p>If base is contained in a non splitable container (like a tab of DockTabbedPane) then, a
   * splitable ancestor will be searched (until the root desktop pane is reached) to apply
   * splitting.
   *
   ...
   * @param proportion proportion of the initial dockable space taken by the new dockable a negative
   ...
   */
  public void split(
      Dockable base, Dockable newDockable, DockingConstants.Split position, double proportion) {
    /*
    ...
    DockingActionEvent dae =
        new DockingActionSplitDockableEvent(
            this, newDockable, currentLocation, newState.getLocation(), base, position, 0.5f);
    ...
    if (baseTab != null) {
      ...
      DockingUtilities.replaceChild(
          ((Component) baseTab).getParent(), ((Component) baseTab), split);
      ...
      DockingUtilities.replaceChild(
          ((Component) baseContainer).getParent(), ((Component) baseContainer), split);
  ...
  /**
   * Replaces the base component by a split pane oriented according to position, and put dockable at
   * position.
   *
   * <p><b>Note : </b> This method is for DockableContainers. API users should use the {@link
   * #split(Dockable,Dockable,DockingConstants.Split) split} method.
   *
  ...
  public void splitComponent(Component base, Dockable dockable, DockingConstants.Split position) {
    ...
    DockingActionEvent dae =
        new DockingActionSplitComponentEvent(
            this, dockable, currentLocation, newState.getLocation(), base, position, 0.5f);
  ...
  /**
   * Replaces the base component by a split pane oriented according to position, and put the
   * dockablesContainer at position.
   *
   * <p><b>Note : </b> This method is for DockableContainers. API users should use the {@link
   * #split(Dockable,Dockable,DockingConstants.Split) split} method.
   *
  ...
  public void splitComponent(
      Component base, Container dockablesContainer, DockingConstants.Split position) {
  ...
  private void splitTab(
      Component base, TabbedDockableContainer tdc, DockingConstants.Split position) {
   ...
   *
   * <p>The dockable is inserted in the docking desktop where it was before, if possible. if not, an
   * approximated place will be looked for.
   */
   ...
   *
   * <p>This method shouldn't be used by user applications (only by the framework).
   *
   * <p>To poperly remove a component from the desktop, use the close(Dockable) method instead
   *
   * <p>If the dockable is not visible, an error will occur
   *
   * <p>This method does not unregister the dockable.
   *
   ...
   *
   * <p>Don't call this method directly, as it is meant to be used only by the drag and drop event
   * components.
   */
  public void dropRemove(DockableDragSource dragSource) {
    ...
    if (state != null) {
      ...
      DockableState updatedState =
          new DockableState(
              state, new RelativeDockablePosition(getRelativeAncestorContainer(d), d));
   ...
   *
   * <p>The component must be docked before beeing maximized, otherwise an IllegalArgumentException
   * will be thrown.
   *
   * <p>The opposite method of maximize is restore(Dockable).
   *
  ...
  public void maximize(Dockable dockable) {
    if (dockable.getDockKey().getLocation() != DockableState.Location.DOCKED) {
      ...
      throw new IllegalArgumentException(
          "Dockable isn't currently in the DOCKED state : " + dockable);
    }
    ...
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
    DockingActionEvent dae =
        new DockingActionSimpleStateChangeEvent(
            this, dockable, currentLocation, DockableState.Location.MAXIMIZED);
    ...
    // compound dockable
    SingleDockableContainer currentMaximizedContainer =
    ...
    DockingUtilities.replaceChild(
        dockableContainer.getParent(), dockableContainer, dummyMaximedReplacer);
   ...
   *
   * <p>The dockable must be already maximized (otherwise an IllegalArgumentException will be
   * thrown)
   *
  ...
  public void restore(final Dockable dockable) {
    if (dockable.getDockKey().getLocation() != DockableState.Location.MAXIMIZED) {
      // development error (could have been an assertion).
      throw new IllegalArgumentException(
          "Dockable isn't currently in the MAXIMIZED state : " + dockable);
    }
    ...
    DockingActionEvent dae =
        new DockingActionSimpleStateChangeEvent(
            this, dockable, currentLocation, DockableState.Location.DOCKED);
    ...
    if (dummyMaximedReplacer.getParent() instanceof TabbedDockableContainer) {
      sdc =
          DockableContainerFactory.getFactory()
              .createDockableContainer(
                  dockable, DockableContainerFactory.ParentType.PARENT_TABBED_CONTAINER);
    } else {
      sdc =
          DockableContainerFactory.getFactory()
              .createDockableContainer(
    ...
    DockingUtilities.replaceChild(
        dummyMaximedReplacer.getParent(), dummyMaximedReplacer, (Component) sdc);
   ...
   *
   * <p>When detached, the dockable is centered on the desktop.
   */
  ...
  /**
   * Used only with DOCKED dockables : returns the nearest ancestor container (can be a
   * CompoundDockingPanel if nested, or the DockingPanel).
   */
   ...
   * Detach or attach the dockable from/to the desktop.
   *
   * <p>show the dockable at sceenPosition (when not null) or centered (if screenPosition is null)
   */
  public void setFloating(final Dockable dockable, boolean floating, Point screenPosition) {
    ...
    SingleDockableContainer dockableContainer =
        DockingUtilities.findSingleDockableContainer(dockable);
    ...
    if (floating) {
      ...
      if (location == DockableState.Location.FLOATING) {
        ...
        DockingActionEvent dae =
            new DockingActionSimpleStateChangeEvent(
                this, dockable, currentLocation, DockableState.Location.FLOATING);
        if (isDockingActionAccepted(dae, event)) {
          ...
          if (dockable instanceof CompoundDockable) {
            updateCompoundChildrenState(
                (CompoundDockable) dockable, DockableState.Location.FLOATING);
          }
      ...
      DockableState newState =
          new DockableState(
              this, dockable, previousState.getLocation(), previousState.getPosition());
      DockableStateWillChangeEvent event = new DockableStateWillChangeEvent(currentState, newState);
      DockingActionEvent dae;
      if (currentState != null && currentState.isFloating()) {
        if (previousState.isDocked()) {
          ...
          dae =
              RelativeDockingUtilities.getInsertionDockingAction(
                  relativeAncestor, dockable, currentState, newState);
        } else if (previousState.isHidden()) { // it was hidden
          dae =
              new DockingActionSimpleStateChangeEvent(
                  this, dockable, DockableState.Location.FLOATING, DockableState.Location.HIDDEN);
        } else {
   ...
   *
   * <p>displays the component at sceenPosition (when not null) or centered (if screenPosition is
   * null)
   */
    ...
    if (!currentState.isDocked()) {
      ...
      if (checkDockableStateWillChange(tdc, DockableState.Location.FLOATING)) {
        RelativeDockablePosition position =
            new RelativeDockablePosition(dockingPanel, firstDockable);
  ...
  private boolean checkDockableStateWillChange(
      TabbedDockableContainer tdc, DockableState.Location futureLocation) {
    for (int i = 0; i < tdc.getTabCount(); i++) {
      Dockable d = tdc.getDockableAt(i);
  ...
  /**
   * Returns the previous state of a hidden or floating dockable (when that dockable was in the
   * DOCKED state.
   *
   * @since 2.0.4
   */
  ...
  private DockableState removePreviousFloatingState(Dockable dockable) {
    if (dockable instanceof CompoundDockable) {
      ...
      ArrayList children =
          DockingUtilities.findCompoundDockableChildren((CompoundDockable) dockable);
  ...
  private void storePreviousFloatingState(Dockable dockable, DockableState state) {
    ...
    if (dockable instanceof CompoundDockable) {
      ...
      for (int i = 0; i < children.size(); i++) {
        Dockable d = (Dockable) children.get(i);
        previousFloatingDockableStates.put(
            d, new DockableState(this, d, state.getLocation(), state.getPosition()));
      }
    }
  }
   ...
   *
   * <p>This method is protected to give access to implementors wanting to customize its look and
   * feel (for example : removing the title bar by setting it to "undecorated").
   *
   * <p>The old API has been removed (it returned a JDialog) as the DockingDesktop can only work
   * with FloatingDockableContainers.
   */
          ...
          if (tparent.getTabCount() == 1) { // no more use for tabs
            ...
            if (floating) {
              ...
              lastContainer =
                  DockableContainerFactory.getFactory()
                      .createDockableContainer(
                          last, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
            }
            lastContainer.installDocking(this);

            DockingUtilities.replaceChild(
                ((Component) tparent).getParent(), (Component) tparent, (Component) lastContainer);
          }
        } else if (parent == dockingPanel) { // no more views to show
          dockingPanel.remove(0);
        } else if (SwingUtilities.getWindowAncestor(parent) instanceof FloatingDockableContainer) {
   ...
   *
   * <p>the dockable remains registered and can later be shown again at the same location, using
   * addDockable(Dockable, RelativeDockablePosition).
   *
   * <p>To have access to the RelativeDockablePosition at closing time, one has to install a
   * DockableEventListener like this :
   *
   * <pre>
  ...
  public void close(Dockable dockable) {
    ...
    RelativeDockablePosition position = new RelativeDockablePosition(dockingPanel, dockable);
    DockableState newState =
        new DockableState(this, dockable, DockableState.Location.CLOSED, position);
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
    ...
    if (dockingPanel.isAncestorOf(dockable.getComponent())) {
      ...
      if (isDockingActionAccepted(dae, dswe)) {
        ...
        context.setDockableState(
            dockable,
            new DockableState(
                this, dockable, DockableState.Location.CLOSED, currentState.getPosition()));
        dockable.getDockKey().setLocation(DockableState.Location.CLOSED);
        fireDockingAction(dae, new DockableStateChangeEvent(currentState, newState));
      }
  ...
  /**
   * Iterates through this compound children and properly close them (without removing them from
   * their parent container)
   */
  private void updateCompoundChildrenState(
      CompoundDockable cDockable, DockableState.Location state) {
   ...
   *
   * <p>Note : is a dockable is not allowed to close (key.isCloseEnabled() returns false) then it
   * won't be closed by this method.
   *
   * <p>If the 'exception' dockable isn't contained in a tab, then nothing will happen.
   */
  ...
  /**
   * Convenience method to close all dockables of the tabbedContainer containing "base" (including
   * the base dockable).
   *
   * <p>see also {@link #closeAllOtherDockablesInTab(Dockable)}.
   *
   * <p>Note : is a dockable is not allowed to close (key.isCloseEnabled() returns false) then it
   * won't be closed by this method.
   *
   * <p>If the 'base' dockable isn't contained in a tab, then nothing will happen.
   */
  ...
  public void setAutoHide(Dockable dockable, boolean hide) {
    ...
    if (hide) {
      ...
      DockableState newState =
          new DockableState(this, dockable, DockableState.Location.HIDDEN, position);
      DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
      DockingActionEvent dae =
          new DockingActionSimpleStateChangeEvent(
              this, dockable, currentLocation, DockableState.Location.HIDDEN);
   ...
   *
   * <p>Visibility states are [DockableState.CLOSED, DOCKED, HIDDEN]
   *
  ...
  /**
   * This method is used by DockableContainers in order to register their DockableDragSource(s) to
   * the OldDragControler.
   *
   * <p>Adds a MouseListener and a MouseMotionListener to the Component-DockableDragSource <code>
   * source</code>. These listeners are used to perform start-dragging-docking actions over the
   * source component
   */
  ...
  /**
   * This method is used by DockableContainers in order to unregister their DockableDragSource(s) to
   * the OldDragControler.
   *
   * <p>Removes the MouseListener and MouseMotionListener added on installDockableDragSource()
   *
   * @see #installDockableDragSource(DockableDragSource)
   */
   ...
   *
   * <p>This method should be invoked when the component is realized (visible) due to Split Panes
   * implementation.
   */
   ...
   *
   * <p>As of version 2.1, this method delegates the work to DockingContext.writeXML, which will
  ...
  void writeDesktopNode(PrintWriter out) throws IOException {
    ...
    out.println(
        "<DockingPanel x=\""
            + (int) windowLocation.getX()
            + "\" y=\""
            + (int) windowLocation.getY()
            + "\" width=\""
            + desktopWindow.getWidth()
            + "\" height=\""
  ...
  private void xmlWriteFloating(PrintWriter out) throws IOException {
    ...
    ArrayList<Dockable> floatingDockables =
        context.getDockablesByState(this, DockableState.Location.FLOATING);
  ...
  private void xmlWriteCompoundDockable(CompoundDockable dockable, PrintWriter out)
      throws IOException {
    out.println("<Dockable compound=\"true\">");
    DockKey key = dockable.getDockKey();
    out.println("<Key dockName=\"" + key.getKey() + "\"/>");
  ...
  private void xmlWriteSplit(SplitContainer split, PrintWriter out) throws IOException {
    ...
    if (split.getOrientation() == JSplitPane.HORIZONTAL_SPLIT) {
      location =
          split.getLeftComponent().getWidth()
    ...
    out.println(
        "<Split orientation=\"" + split.getOrientation() + "\" location=\"" + location + "\">");
  ...
  /**
   * TabGroups are the memory of tabs (even for hidden or floating dockables) used to put them back
   ...
   *
   * <p>When the method returns, the desktop is totally reconfigured with posiibly different
   * dockable at different positions.
   *
   * <p><b>Note : </b> The <code>DockKey</code>s of the stream must be registered with the {@link
   * #registerDockable(Dockable) registerDockable} method, prior readXML. <br>
   * This is the case if the desktop is already open and dockables laid out, but might not be the
   * case if this method is used at application startup to populate an empty desktop.
   *
   * <p>Dismisses all visible dockables (docked and auto-hidden), and clear their DockableState.
   *
   * <p>The stream is not closed at the end of the operation.
   *
  ...
  private void xmlBuildAutoHideNode(AutoHideButtonPanel borderPanel, Element dockableElt)
    ...
    context.setDockableState(
        dockable, new DockableState(this, dockable, DockableState.Location.HIDDEN, position));
    ...
    if (dockable instanceof CompoundDockable) {
      ...
      xmlBuildCompoundDockable(
          (CompoundDockable) dockable, dockableElt, DockableState.Location.HIDDEN);
    }
  }
  ...
  private void xmlBuildFloatingNode(NodeList dockables, Rectangle bounds) throws SAXException {
    ...
    for (int i = 0; i < dockables.getLength(); i++) {
      ...
      DockableState state =
          new DockableState(this, dockable, DockableState.getLocationFromInt(istate), position);
      ...
      if (fdc == null) {
        ...
        if (tdc.getTabCount() == 0) {
          ...
          DockingUtilities.replaceChild(
              ((Component) base).getParent(), (Component) base, (Component) tdc);
      ...
      context.setDockableState(
          dockable, new DockableState(this, dockable, DockableState.Location.FLOATING, position));
      dockable.getDockKey().setLocation(DockableState.Location.FLOATING);

      if (dockable instanceof CompoundDockable) {
        // check for children and build them
        xmlBuildCompoundDockable(
            (CompoundDockable) dockable, dockableElt, DockableState.Location.FLOATING);
      }
    }
  ...
  private Component xmlCreateComponent(Element elt, DockableState.Location dockableLocation)
    ...
    if (elt.getNodeName().equals("Split")) {
      ...
      SingleDockableContainer sdc =
          DockableContainerFactory.getFactory()
              .createDockableContainer(
                  d, DockableContainerFactory.ParentType.PARENT_SPLIT_CONTAINER);
  ...
  private void xmlBuildCompoundDockable(
      CompoundDockable dockable, Element compoundElt, DockableState.Location dockableLocation)
      throws SAXException {
  ...
  private TabbedDockableContainer xmlBuildTabbedDockableContainer(
    ...
    TabbedDockableContainer tdc =
        DockableContainerFactory.getFactory().createTabbedDockableContainer();
    tdc.installDocking(this);
   ...
   *
   * <p>This method can be used mainly in two situations :
   *
   * <ul>
   *   <li>the desktop is empty (when not empty, you can use split() or createTab() to add a
   *       dockable relatively to another)
   *   <li>the dockable is currently closed, and it must be shown again
   *   <li>note that if the desktop is not empty, the dockable will be added in the bottom of the
   *       desktop (equivalent to addDockable(dockable 0,0.8,1,0.2)).
   * </ul>
   */
  ...
  /**
   * Adds a dockable in the docking desktop, and tries to respect the relative positionning
   * provided.
   *
   * <p>This method is used to reposition a closed dockable at its previous location on the desktop.
   * As it relies on the Component.findComponentAt(Point) method , the desktop must already be
   * visible.
   *
   * <p>The preferred way to obtain a particular visual docking configuration is to use a
   * combination of add(), split() and createTab() methods, as these methods do not rely on an
   * interpretation (and approximation) of constraints.
   *
   * <p>Precision of constraints : as the docking management is based on a mix of horizontal and
   * vertical splitting zones, it is not always possible to respect the constraints given.
   *
   * <p>The current implementation will do the following :
   *
   * <ul>
   ...
   *   <li>sub-split the splitter (horizontally or vertically) zone and position the Dockable in the
   *       most appropriate zone (top, left, bottom, right).
   * </ul>
   *
  ...
  public void addDockable(Dockable dockable, RelativeDockablePosition position) {
    ...
    DockableState newState =
        new DockableState(this, dockable, DockableState.Location.DOCKED, position);
    ...
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
    DockingActionEvent dae =
        new DockingActionAddDockableEvent(
            this, dockable, currentLocation, newState.getLocation(), dockingPanel);
    if (isDockingActionAccepted(dae, dswe)) {
      @SuppressWarnings("deprecation")
      SingleDockableContainer sdc =
          RelativeDockingUtilities.insertDockable(dockingPanel, dockable, position);
      sdc.installDocking(this);
   ...
   *
   * <p>Useful for example to reserve some space on a desktop by using a "placeholder' when other
   * dockables aren't visible (Multiple Tabbed Document Interface with always a dockable visible
   * even when no document is loaded).
   *
   * <p>Implementation note : this method assumes only the "base" dockable is visible (the replacer
   * must be in the CLOSED state). It also assumes the base dockable isn't a compound dockable.
   * These limitations will be removed in a later release.
   *
   * <p>There is currently no DnD gesture associated to this action (although the "HotSwap" gesture
   * would be a good candidate). So this method currently doesn't trigger any event (state change,
   * action), yet it could change later.
   *
   * @since 2.1
  ...
  public void replace(Dockable base, Dockable replacer) {
    ...
    DockableState newBaseState =
        new DockableState(replacerState.getDesktop(), base, replacerState.getLocation());
    DockableState newReplacerState =
        new DockableState(baseState.getDesktop(), replacer, baseState.getLocation());
   ...
   *
   * <p>This is the method to start a nesting hierarchy inside a compound dockable (once the first
   * dockable is added, you add subsequent dockables with standard split/createTab methods)
   *
   * <ul>
  ...
  public void addDockable(CompoundDockable base, Dockable dockable) {
    ...
    DockingActionEvent dae =
        new DockingActionAddDockableEvent(
            this, dockable, currentLocation, newState.getLocation(), panel);

    if (isDockingActionAccepted(dae, dswe)) {
      DockableContainer dockableContainer =
          DockableContainerFactory.getFactory()
              .createDockableContainer(
                  dockable, DockableContainerFactory.ParentType.PARENT_DESKTOP);
      dockableContainer.installDocking(this);
  ...
  /**
   * Evaluates and returns the preferred size of the desktop, according to its contents and
   * auto-hide borders.
   */
   ...
   *
   * <p>DockableStateChange Events are triggered after the state change.
   *
   * <p>As of version 2.0 of the framework, this method can also be replaced by adding a
   * propertychangeListener on the DockKey object (and react to its DockableState property).
   */
   ...
   *
   * <p>DockableStateWillChange Events are triggered <b>before</b> the state change, and are
   * vetoable.
   */
   ...
   *
   * <p>DockingAction Events are triggered before and after any docking action (split, tab, close,
   * hide...) and allow for precise tracking (and vetoing) of these operations.
   *
   ...
   *
   * <p>DockableSelection Events are triggered when a dockable takes the focus.
   */
   ...
   *
   * <p>this method should be called at startup time (when the dockable isn't yet displayed). to
   * toggle a dockable from docked to auto-hide, please use setAutoHide(Dockable, boolean) instead.
   *
   * <p>However, this method will not fail if the dockable is already displayed : in that case, it
   * will delegate autohide to setAutoHide(Dockable, boolean), thus dropping the dockedPosition
   * argument.
   *
   * @param dockable the dockable to add (to select the border where the dockable will be
   *     positionned, use the DockKey.setAutoHideBorder mehod
   * @param dockedPosition relative positionning (may be null) indicating where to dock the dockable
   *     when leaving its auto-hide border.
   */
  public void addHiddenDockable(Dockable dockable, RelativeDockablePosition dockedPosition) {
    ...
    DockableState newState =
        new DockableState(this, dockable, DockableState.Location.HIDDEN, dockedPosition);
    DockableStateWillChangeEvent dswe = new DockableStateWillChangeEvent(currentState, newState);
    DockingActionEvent dae =
        new DockingActionSimpleStateChangeEvent(
            this, dockable, currentLocation, DockableState.Location.HIDDEN);
   ...
   *
   * <p>Note that this method works by modifying the divider location of the nearest SplitContainer
   * ancestor of the dockable (the first splitted horizontally).
   *
   * <p>As this method must be called when the desktop is visible, a simple way to do this is to add
   * a window listener to the parent frame of the desktop, and call this method from the
   * windowOpened(WindowEvent) notification.
   *
   * @param width new width (if between 0 and 1, width is taken as a proportional width, otherwise
   *     it is a pixel width.
   */
   ...
   *
   * <p>Note that this method works by modifying the divider location of the nearest SplitContainer
   * ancestor of the dockable (the first splitted vertically).
   *
   * <p>As this method must be called when the desktop is visible, a simple way to do this is to add
   * a window listener to the parent frame of the desktop, and call this method from the
   * windowOpened(WindowEvent) notification.
  ...
  /**
   * Updates the resizing behaviour of the desktop in order to resize this dockable (and keep others
   * at fixed size).
   *
   * <p>This method should be called at least once (prior to showing the desktop) with an already
   * docked dockable.
   *
   * <p>If the dockable doesn't belong to the desktop containment hierarchy, this method will do
   * nothing.
   *
   ...
   *
   * <p>It is used to have a memory of grouped (tabbed) dockables in order to keep the group
   * together when dockable are restored from auto-hide mode.
   *
   * <p>This method is generally called by the tabbed container management, and not directly by the
   * developper.
   *
   * <p>However, there is a case where calling this method can be usefull : when, at startup, a
   * desktop is built with multiple hidden dockables, and the developper wants them to be grouped in
   * a tab container when they are restored to the desktop.
   *
   * <p>note that the method is symetric when a group is empty : it such a case base and newTab args
   * can be swapped.
   *
   ...
   *
   * <p>This method gives a chance to the developer to override expand panel creation and provide a
   * custom subclass.
   */
   ...
   *
   * <p>Warning : this method should only be used by the framework itself : changing a context
  ...
  /**
   * combines a docking action event and vetoable state change event to accept or reject a docking
   * action.
   */
  private boolean isDockingActionAccepted(
      DockingActionEvent dae, DockableStateWillChangeEvent dse) {
