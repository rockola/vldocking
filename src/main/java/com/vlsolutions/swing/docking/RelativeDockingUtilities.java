...
/**
 * This class provides an algorithm for finding (and then inserting) the most suitable place to show
 * a dockable, based on a relative positionning.
 *
 * @author Lilian Chamontin, vlsolutions.
 * @version 1.0
 * @update 2005/10/10 Lilian Chamontin : improved heuristics of resizing (best matching of the
 *     appropriate SplitContainer)
...
class RelativeDockingUtilities {
  ...
  /** try to find the best suitable SplitContainer from where insertion will be done. */
  private static SplitContainer findBestContainer(
      Container ancestorContainer, RelativeDockablePosition position) {
    ...
    if (splitComp instanceof SplitContainer) { // 2006/09/12
      ...
      while (found && splitComp.getParent() instanceof SplitContainer) {
        ...
        boolean sameOrientation =
            ((SplitContainer) splitComp).getOrientation() == parentSplit.getOrientation();
        if (sameOrientation) {
    ...
    if (splitComp instanceof SplitContainer) {
      ...
      //          boolean anchoredBottom = Math.abs(y + h - dockingPanel.getY() -
      // dockingPanel.getHeight()) < 5;
      //          boolean anchoredRight = Math.abs(x + w - dockingPanel.getX() -
      // dockingPanel.getWidth()) < 5;
      ...
      return findBetterContainer(
          (SplitContainer) splitComp,
          ancestorContainer,
          anchoredTop,
          anchoredLeft,
          anchoredBottom,
  ...
  private static SplitContainer findBetterContainer(
      SplitContainer split,
      Container dockingPanel,
      boolean anchoredTop,
      boolean anchoredLeft,
      boolean anchoredBottom,
    ...
    // and there are two horizontal splits there, we might return the internal split (not full
    // width),
  ...
  */
   ...
   * @since 2004/04/24
   */
  private static void hSplitAndResize(
      Component base, Component left, Component right, double proportion) {
   ...
   * @since 2004/04/24
   */
  private static void vSplitAndResize(
      Component base, Component top, Component bottom, double proportion) {
    Container parent = base.getParent();
   ...
   */
  public static SingleDockableContainer insertDockable(
      Container relativeAncestorContainer, Dockable dockable, RelativeDockablePosition position) {
      ...
      if (split != null) {
        ...
        Point leftCenter =
            SwingUtilities.convertPoint(
                left, left.getWidth() / 2, left.getHeight() / 2, relativeAncestorContainer);
        Point rightCenter =
            SwingUtilities.convertPoint(
                right, right.getWidth() / 2, right.getHeight() / 2, relativeAncestorContainer);
        int leftDist =
            (leftCenter.x - x) * (leftCenter.x - x) + (leftCenter.y - y) * (leftCenter.y);
        int rightDist =
            (rightCenter.x - x) * (rightCenter.x - x) + (rightCenter.y - y) * (rightCenter.y);
        ...
        /**
         * this will happen only when anchors are not too strict (like top+right) for wider anchors
         * (left-top-right) this ration is not used as superflous
         */
          ...
          if (anchoredTop) {
              ...
              if (anchoredBottom) { // TLB = full left
                ...
                if (splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
                  ...
                  SwingUtilities.invokeLater(
                      new SplitResizer(newSplit, h / (float) split.getHeight()));
                } else {
                  SplitContainer newSplit = new SplitContainer(JSplitPane.HORIZONTAL_SPLIT);
            ...
            if (anchoredLeft) { // BL
              if (anchoredRight) { // BLR = full bottom
                ...
                if (splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
                  ...
                  SwingUtilities.invokeLater(
                      new SplitResizer(newSplit, 1 - h / (float) split.getHeight()));
                } else {
                  SplitContainer newSplit = new SplitContainer(JSplitPane.HORIZONTAL_SPLIT);
                  DockingUtilities.replaceChild(split, bottom, newSplit);
                  newSplit.setLeftComponent((Component) dockableContainer);
            ...
            if (anchoredRight) { // 2005/10/10
              ...
              if (bestComp == top) {
                ...
                SwingUtilities.invokeLater(
                    new SplitResizer(
                        split, (split.getHeight() - yBottom) / (float) split.getHeight()));
                SwingUtilities.invokeLater(
                    new SplitResizer(newSplit, yTop / (float) split.getHeight()));
              } else {
                newSplit.setBottomComponent(bestComp);
                newSplit.setTopComponent((Component) dockableContainer);
                SwingUtilities.invokeLater(
                    new SplitResizer(split, yTop / (float) split.getHeight()));
          ...
          if (anchoredTop) {
            if (anchoredLeft) {
              ...
              if (bestComp == left) {
                ...
                SwingUtilities.invokeLater(
                    new SplitResizer(
                        split, (split.getWidth() - xRight) / (float) split.getWidth()));
                SwingUtilities.invokeLater(
                    new SplitResizer(newSplit, xLeft / (float) split.getWidth()));
              } else {
                newSplit.setRightComponent(bestComp);
                newSplit.setLeftComponent((Component) dockableContainer);
                SwingUtilities.invokeLater(
                    new SplitResizer(split, xLeft / (float) split.getWidth()));
            ...
            if (anchoredRight) {
              ...
              DockingUtilities.replaceChild(split.getParent(), split, newSplit);
              newSplit.setTopComponent((Component) dockableContainer);
              newSplit.setBottomComponent(
                  split); // todo : check this case :  why is the new component on top
              SwingUtilities.invokeLater(new SplitResizer(newSplit, proportionH));
            } else {
              // just left
              if (splitWidthRatio < 0.1f && split.getHeight() > h) { // 2006/09/12
        ...
        if (anchoredTop) {
          if (anchoredLeft) {
            ...
            if (anchoredBottom) { // top + right + bottom == on the right
              hSplitAndResize(
                  singleComp, singleComp, (Component) dockableContainer, 1 - proportionW);
            } else { // top + right
              if (w > h) { // on top
                vSplitAndResize(singleComp, (Component) dockableContainer, singleComp, proportionH);
              } else { // on the right
          ...
          if (anchoredBottom) { // left + bot
            if (anchoredRight) { // == bottom
              vSplitAndResize(
                  singleComp, singleComp, (Component) dockableContainer, 1 - proportionH);
            } else { // left + bottom
              if (w > h) { // bottom
                vSplitAndResize(
                    singleComp, singleComp, (Component) dockableContainer, 1 - proportionH);
    ...
    // DockingActionEvent action = getInsertionDockingAction(relativeAncestorContainer, dockable,
    // position);
  ...
  /**
   * returns a docking action event corresponding to the insertion point and method of the dockable.
   */
  public static DockingActionEvent getInsertionDockingAction(
      Container relativeAncestorContainer,
      Dockable dockable,
    ...
    SingleDockableContainer dockableContainer = null;
    if (relativeAncestorContainer.getComponentCount() == 0) { // empty docking panel
      return new DockingActionAddDockableEvent(
          desktop, dockable, initialState, nextState, relativeAncestorContainer);
    } else {
      // convert relative positionning to current positionning (depending
      // on the current size of dockingPanel)
      int x = (int) (position.getX() * relativeAncestorContainer.getWidth());
      ...
      if (split != null) {
        ...
        if (split.getOrientation() == JSplitPane.VERTICAL_SPLIT) {
          ...
          if (anchoredTop) {
            if (anchoredLeft) {
              if (anchoredBottom) {
                return new DockingActionSplitComponentEvent(
                    desktop,
                    dockable,
                    initialState,
                    nextState,
  ...
  public static SingleDockableContainer applyDockingAction(
      Dockable dockable, DockingActionEvent action) {
