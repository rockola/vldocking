...
/**
 * An utility class used to delegate the border drag/drop scanning for docking ( since the same code
 * is used in many DockDropReceivers).
 *
 * <p>This class is only meant for API Extenders.
 *
 * <p>Taking a DockDragEvent or DockDropEvent, check if mouse is near of one border. If so, accepts
 * the drag or drop operation.
 *
 * @author Lilian Chamontin, vlsolutions.
 * @version 1.0
 * @update 2005/10/21 Lilian Chamontin : updated the shape to show real size of drop (and not 20
...
public class BorderSplitter {
  ...
  /**
   * This method should be overriden if the delegator doesn't belong to the docking containment
   * hierarchy after drop acceptance.
   *
   * <p>Here is a case :
   *
   * <UL>
   ...
   * </UL>
   *
   * <p>Then, tabpane will be replaced by a simple DockableContainer on drop acceptance (because
   * only one tab left).
   *
   * <p>And DockingDesktop.splitContainer(Component base, Split position) cannot be called with
   * tabpane as "base" (instead, it should be called with the replacer of tabpane, or use the other
   * split method which takes a dockable as base.
   */
  public void split(
      DockDropEvent event, Container draggedContainer, DockingConstants.Split position) {
    /* standard splitting */
  ...
  private void acceptSplit(DockDropEvent event, DockingConstants.Split position) {
    ...
    if (tabAncestor != null
        && tabAncestor.getTabCount() == 2
        && tabAncestor == delegator) { // 2007//11/15
    ...
    if (container instanceof TabbedDockableContainer) {
      ...
      // if (tabAncestor != null && tabAncestor.getTabCount() == 1 && tabAncestor ==
      // delegator){//2007//11/15
      if (tabAncestor != null
          && tabAncestor.getTabCount() == 0
          && tabAncestor == delegator) { // 2007//11/15
  ...
  private void acceptDragSplit(DockDragEvent event, DockingConstants.Split position, Shape shape) {
    ...
    DockableState.Location borderSpliterLocation =
        DockingUtilities.getDockableLocationFromHierarchy(delegator);
